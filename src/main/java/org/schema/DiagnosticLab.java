...
public class DiagnosticLab extends MedicalOrganization {
  ...
  static final class DiagnosticLabThingBuilder implements Builder {
     ...
     * A diagnostic test or procedure offered by this lab.
     */
    @NotNull public Builder availableTest(MedicalTest medicalTest) {
...
Note: Publishers should be aware that applications designed to use specific schema.org properties (e.g. http://schema.org/width, http://schema.org/color, http://schema.org/gtin13, ...) will typically expect such data to be provided using those properties, rather than using the generic property/value mechanism.
  ...
  public interface Builder extends ThingBuilder<DiagnosticLab> {
    @NotNull Builder availableTest(MedicalTest medicalTest);
    @NotNull Builder availableTest(MedicalTest.Builder medicalTest);
