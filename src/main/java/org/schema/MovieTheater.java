...
public class MovieTheater extends EntertainmentBusiness {
   ...
   */
  public static final class MovieTheaterThingBuilder implements Builder {
    /**
     ...
     * The number of screens in the movie theater.
     */
    public Builder screenCount(String screenCount) {
      if (this.screenCount == null) this.screenCount = new Number();
...
Note: Publishers should be aware that applications designed to use specific schema.org properties (e.g. http://schema.org/width, http://schema.org/color, http://schema.org/gtin13, ...) will typically expect such data to be provided using those properties, rather than using the generic property/value mechanism.
  ...
  Builder screenCount(Integer integer);
  Builder screenCount(Long screenCount);
  Builder screenCount(Float screenCount);
  Builder screenCount(Double screenCount);
  Builder screenCount(String screenCount);
