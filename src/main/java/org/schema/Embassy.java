...
public class Embassy extends GovernmentBuilding {
   ...
   */
  static final class EmbassyThingBuilder implements Builder {
    /**
     * Creates new {@link Embassy} instance.
     */
    public Embassy build() {
      return new Embassy(openingHours, address, aggregateRating, containedIn, event, faxNumber, geo, globalLocationNumber, isicV4, logo, hasMap, openingHoursSpecification, photo, review, telephone, additionalProperty, additionalType, alternateName, description, mainEntityOfPage, name, sameAs, url, potentialAction, id);
    }
    /**
...
Note: Publishers should be aware that applications designed to use specific schema.org properties (e.g. http://schema.org/width, http://schema.org/color, http://schema.org/gtin13, ...) will typically expect such data to be provided using those properties, rather than using the generic property/value mechanism.
  ...
  public interface Builder extends ThingBuilder<Embassy> {
