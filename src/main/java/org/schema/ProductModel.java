...
Note: Publishers should be aware that applications designed to use specific schema.org properties (e.g. http://schema.org/width, http://schema.org/color, http://schema.org/gtin13, ...) will typically expect such data to be provided using those properties, rather than using the generic property/value mechanism.
  ...
  public interface Builder extends ThingBuilder<ProductModel> {
    @NotNull Builder isVariantOf(@NotNull ProductModel productModel);
    @NotNull Builder isVariantOf(@NotNull ProductModel.Builder productModel);
    @NotNull Builder predecessorOf(@NotNull ProductModel productModel);
    @NotNull Builder predecessorOf(@NotNull ProductModel.Builder productModel);
    @NotNull Builder successorOf(@NotNull ProductModel productModel);
    @NotNull Builder successorOf(@NotNull ProductModel.Builder productModel);
