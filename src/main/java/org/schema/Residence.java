...
public class Residence extends Place {
   ...
   */
  public static final class ResidenceThingBuilder implements Builder {
    /**
     * Creates new {@link Residence} instance.
     */
    public Residence build() {
      return new Residence(address, aggregateRating, containedIn, event, faxNumber, geo, globalLocationNumber, isicV4, logo, hasMap, openingHoursSpecification, photo, review, telephone, additionalProperty, additionalType, alternateName, description, mainEntityOfPage, name, sameAs, url, potentialAction, id);
    }
    /**
...
Note: Publishers should be aware that applications designed to use specific schema.org properties (e.g. http://schema.org/width, http://schema.org/color, http://schema.org/gtin13, ...) will typically expect such data to be provided using those properties, rather than using the generic property/value mechanism.
  ...
  public interface Builder extends ThingBuilder<Residence> {
