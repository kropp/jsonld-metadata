...
class MutableAboutPage {
  ...
  var breadcrumb: Any by map
  fun breadcrumbBreadcrumbList(builder: MutableBreadcrumbList.() -> Unit) { breadcrumb = MutableBreadcrumbList().apply(builder).build() }
  var lastReviewed: java.util.Date by map
  var mainContentOfPage: WebPageElement by map
  fun mainContentOfPage(builder: MutableWebPageElement.() -> Unit) { mainContentOfPage = MutableWebPageElement().apply(builder).build() }
  var primaryImageOfPage: ImageObject by map
  fun primaryImageOfPage(builder: MutableImageObject.() -> Unit) { primaryImageOfPage = MutableImageObject().apply(builder).build() }
  var relatedLink: String by map
  var reviewedBy: Any by map
  fun reviewedByOrganization(builder: MutableOrganization.() -> Unit) { reviewedBy = MutableOrganization().apply(builder).build() }
  fun reviewedByPerson(builder: MutablePerson.() -> Unit) { reviewedBy = MutablePerson().apply(builder).build() }
  var significantLink: String by map
  var specialty: Specialty by map
