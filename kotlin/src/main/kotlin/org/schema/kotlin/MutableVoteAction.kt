...
class MutableVoteAction {
  ...
  var candidate: Person by map
  fun candidate(builder: MutablePerson.() -> Unit) { candidate = MutablePerson().apply(builder).build() }
  var actionOption: Any by map
  fun actionOptionThing(builder: MutableThing.() -> Unit) { actionOption = MutableThing().apply(builder).build() }
