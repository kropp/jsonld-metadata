...
class MutableBroadcastService {
  ...
  var broadcaster: Organization by map
  fun broadcaster(builder: MutableOrganization.() -> Unit) { broadcaster = MutableOrganization().apply(builder).build() }
  var parentService: BroadcastService by map
  fun parentService(builder: MutableBroadcastService.() -> Unit) { parentService = MutableBroadcastService().apply(builder).build() }
  var videoFormat: String by map
  var broadcastAffiliateOf: Organization by map
  fun broadcastAffiliateOf(builder: MutableOrganization.() -> Unit) { broadcastAffiliateOf = MutableOrganization().apply(builder).build() }
  var broadcastDisplayName: String by map
  var broadcastTimezone: String by map
