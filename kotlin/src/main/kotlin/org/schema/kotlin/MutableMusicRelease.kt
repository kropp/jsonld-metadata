...
class MutableMusicRelease {
  ...
  var catalogNumber: String by map
  var creditedTo: Any by map
  fun creditedToOrganization(builder: MutableOrganization.() -> Unit) { creditedTo = MutableOrganization().apply(builder).build() }
  fun creditedToPerson(builder: MutablePerson.() -> Unit) { creditedTo = MutablePerson().apply(builder).build() }
  var musicReleaseFormat: MusicReleaseFormatType by map
  var recordLabel: Organization by map
  fun recordLabel(builder: MutableOrganization.() -> Unit) { recordLabel = MutableOrganization().apply(builder).build() }
  var releaseOf: MusicAlbum by map
  fun releaseOf(builder: MutableMusicAlbum.() -> Unit) { releaseOf = MutableMusicAlbum().apply(builder).build() }
  var numTracks: Int by map
  var track: Any by map
  fun trackItemList(builder: MutableItemList.() -> Unit) { track = MutableItemList().apply(builder).build() }
  fun trackMusicRecording(builder: MutableMusicRecording.() -> Unit) { track = MutableMusicRecording().apply(builder).build() }
