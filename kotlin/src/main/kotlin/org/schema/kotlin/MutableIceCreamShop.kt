...
class MutableIceCreamShop {
  ...
  var acceptsReservations: Any by map
  var menu: Any by map
  fun menuMenu(builder: MutableMenu.() -> Unit) { menu = MutableMenu().apply(builder).build() }
  var servesCuisine: String by map
  var starRating: Rating by map
  fun starRating(builder: MutableRating.() -> Unit) { starRating = MutableRating().apply(builder).build() }
