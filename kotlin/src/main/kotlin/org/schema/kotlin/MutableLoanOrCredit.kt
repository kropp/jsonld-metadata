...
class MutableLoanOrCredit {
  ...
  var loanTerm: QuantitativeValue by map
  fun loanTerm(builder: MutableQuantitativeValue.() -> Unit) { loanTerm = MutableQuantitativeValue().apply(builder).build() }
  var requiredCollateral: Any by map
  fun requiredCollateralThing(builder: MutableThing.() -> Unit) { requiredCollateral = MutableThing().apply(builder).build() }
