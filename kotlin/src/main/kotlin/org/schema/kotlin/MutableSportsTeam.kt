...
class MutableSportsTeam {
  ...
  var coach: Person by map
  fun coach(builder: MutablePerson.() -> Unit) { coach = MutablePerson().apply(builder).build() }
  var athlete: Person by map
  fun athlete(builder: MutablePerson.() -> Unit) { athlete = MutablePerson().apply(builder).build() }
  var sport: String by map
