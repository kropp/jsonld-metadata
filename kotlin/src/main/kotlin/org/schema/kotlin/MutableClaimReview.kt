...
class MutableClaimReview {
  ...
  var claimReviewed: String by map
  var itemReviewed: Thing by map
  fun itemReviewed(builder: MutableThing.() -> Unit) { itemReviewed = MutableThing().apply(builder).build() }
  var reviewBody: String by map
  var reviewRating: Rating by map
  fun reviewRating(builder: MutableRating.() -> Unit) { reviewRating = MutableRating().apply(builder).build() }
