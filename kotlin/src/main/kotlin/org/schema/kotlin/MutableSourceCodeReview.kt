...
class MutableSourceCodeReview {
  ...
  var state: ReviewState by map
  var reviewer: Person by map
  fun reviewer(builder: MutablePerson.() -> Unit) { reviewer = MutablePerson().apply(builder).build() }
  var watcher: Person by map
  fun watcher(builder: MutablePerson.() -> Unit) { watcher = MutablePerson().apply(builder).build() }
