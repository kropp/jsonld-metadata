...
class MutableEmailMessage {
  ...
  var sender: Any by map
  fun senderAudience(builder: MutableAudience.() -> Unit) { sender = MutableAudience().apply(builder).build() }
  fun senderOrganization(builder: MutableOrganization.() -> Unit) { sender = MutableOrganization().apply(builder).build() }
  fun senderPerson(builder: MutablePerson.() -> Unit) { sender = MutablePerson().apply(builder).build() }
  var dateRead: java.util.Date by map
  var dateReceived: java.util.Date by map
  var dateSent: java.util.Date by map
  var messageAttachment: CreativeWork by map
  fun messageAttachment(builder: MutableCreativeWork.() -> Unit) { messageAttachment = MutableCreativeWork().apply(builder).build() }
