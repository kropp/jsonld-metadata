...
class MutableBroadcastEvent {
  ...
  var broadcastOfEvent: Event by map
  fun broadcastOfEvent(builder: MutableEvent.() -> Unit) { broadcastOfEvent = MutableEvent().apply(builder).build() }
  var isLiveBroadcast: Boolean by map
  var isAccessibleForFree: Boolean by map
  var publishedOn: BroadcastService by map
  fun publishedOn(builder: MutableBroadcastService.() -> Unit) { publishedOn = MutableBroadcastService().apply(builder).build() }
