...
class MutableFollowAction {
  ...
  var followee: Any by map
  fun followeeOrganization(builder: MutableOrganization.() -> Unit) { followee = MutableOrganization().apply(builder).build() }
  fun followeePerson(builder: MutablePerson.() -> Unit) { followee = MutablePerson().apply(builder).build() }
